                ...
                <tr key={c.id} style={{ borderBottom: '0.5px solid #f0efe7' }}>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                      {c.status === 'pending' && (
                        <button className="btn-tlb" style={{ fontSize: 12, padding: '5px 12px' }} disabled={launching === c.id} onClick={() => handleLaunch(c.id)}>
                          {launching === c.id ? 'Launching…' : 'Launch →'}
                        </button>
                      )}
                      {c.status === 'active' && <span style={{ fontSize: 12, color: '#3B6D11' }}>● Live</span>}
                      {c.status === 'paused' && <span style={{ fontSize: 12, color: '#7A5C00' }}>⚠ Paused</span>}
                      <Link href={`/dashboard/campaigns/${c.id}/codes`} style={{ fontSize: 12, color: '#FF6B00', textDecoration: 'none' }}>Codes →</Link>
                      <button onClick={() => handleDelete(c.id, c.name)} disabled={deleting === c.id}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#A32D2D', fontSize: 12, padding: 0 }}>
