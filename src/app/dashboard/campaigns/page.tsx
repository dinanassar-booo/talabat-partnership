...
export default async function CampaignsPage() {
  ...
  const rows = await db
    .select({ c: campaigns, bt: benefitTypes })
    .from(campaigns)
    .leftJoin(benefitTypes, eq(campaigns.benefitTypeId, benefitTypes.id))
    .where(eq(campaigns.partnerId, session.id))
    .orderBy(desc(campaigns.createdAt))

  const totalGmv = rows.reduce((s, r) => s + r.c.budgetUsed, 0)
  const totalBudget = rows.reduce((s, r) => s + r.c.budgetTotal, 0)
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <Link href="/dashboard/benefits">
          <button className="btn-tlb"><Plus size={14} /> New campaign</button>
        </Link>
      </div>
      ...
      {rows.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
          ...
          <p style={{ color: '#888', fontSize: 13, margin: '0 0 16px' }}>Set up your first benefit to start generating GMV from your employee base.</p>
        ...
        <div className="card" style={{ padding: 0 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>{['Campaign', 'Benefit', 'Employees', 'Credit/cycle', 'Budget used', 'Status', ''].map(h => (
              ...
              {rows.map(({ c, bt }) => {
                ...
                return (
                  <tr key={c.id} style={{ borderBottom: '0.5px solid #f0efe7' }}>
                    ...
                    <td style={{ padding: '13px 16px' }}><span className="badge badge-orange">{bt?.name ?? '—'}</span></td>
                    <td style={{ padding: '13px 16px', color: '#5f5e5a' }}>{c.headcount.toLocaleString()}</td>
                    <td style={{ padding: '13px 16px', color: '#5f5e5a' }}>AED {c.creditValue} / {c.cycleType.replace('_', ' ')}</td>
                    <td style={{ padding: '13px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ flex: 1, height: 4, background: '#e0dfd7', borderRadius: 2, minWidth: 60 }}>
                          <div style={{ height: '100%', background: '#FF6B00', borderRadius: 2, width: `${pct}%` }} />
                        </div>
                        <span style={{ color: '#5f5e5a', fontSize: 12, whiteSpace: 'nowrap' }}>{pct}%</span>
                      </div>
                      <div style={{ fontSize: 11, color: '#aaa', marginTop: 2 }}>AED {Math.round(c.budgetUsed).toLocaleString()} / {Math.round(c.budgetTotal).toLocaleString()}</div>
                    </td>
                    <td style={{ padding: '13px 16px' }}><span className={`badge badge-${c.status}`}>{c.status}</span></td>
                    <td style={{ padding: '13px 16px' }}>
                      <Link href={`/dashboard/campaigns/${c.id}`} style={{ color: '#FF6B00', fontSize: 12 }}>View →</Link>
                    </td>
                  </tr>
                )
              })}
