...
export async function triggerVoucherWalletCampaign(params: {
  ...
  const body = {
    ...
    canvas_entry_properties: {
      ...
      title_voucher_en: `AED ${creditValue} from ${partnerName}`,
      title_voucher_ar: `${creditValue} درهم من ${partnerName}`,
      tcs_voucher_en: `AED ${creditValue} voucher from ${partnerName} - valid till {{validity_voucher | date: "%Y-%m-%d" | strip}}`,
      tcs_voucher_ar: `قسيمة ${creditValue} درهم من ${partnerName} - صالحة حتى {{validity_voucher | date: "%Y-%m-%d" | strip}}`,
