...
export default function JoinPage() {
  ...
  useEffect(() => {
    fetch(`/api/join/info?slug=${slug}`)
      .then(r => r.json())
      .then(d => { if (d.error) setNotFound(true); else setCompanyName(d.companyName) })
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: '#F7F6F3' }}>
      <header style={{ background: '#FF6B00', padding: '14px 32px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ color: '#fff', fontWeight: 700, fontSize: 20 }}>talabat</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 54px)', padding: 24 }}>
        <div style={{ width: '100%', maxWidth: 480, textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 600, margin: '0 0 12px' }}>You are enrolled!</h1>
          <p style={{ color: '#5f5e5a', fontSize: 15, lineHeight: 1.7, margin: '0 0 8px' }}>
            We have sent your benefit code to <strong>{companyEmail}</strong>
          </p>
  ...
  const steps = [
    ...
    { title: 'Check your email', desc: 'You will receive a unique benefit code on your work email.' },
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 24px', display: 'grid', gridTemplateColumns: '1fr 420px', gap: 64, alignItems: 'start' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 15, color: '#5f5e5a', lineHeight: 1.7, margin: '0 0 36px', maxWidth: 480 }}>
            {companyName || 'Your employer'} has partnered with talabat to give you exclusive food benefits. Sign up below with your work details to receive your personal benefit code.
          </p>

          <div style={{ background: '#fff', border: '0.5px solid #e0dfd7', borderRadius: 12, padding: '20px 24px', marginBottom: 32 }}>
            <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 8 }}>Do not have a talabat account yet?</div>
            <p style={{ fontSize: 13, color: '#5f5e5a', lineHeight: 1.6, margin: 0 }}>
              No worries. You can sign up on the talabat app. Once you have created an account, come back and complete this form. You will receive your benefit code by email.
            </p>
          </div>
